...
public class BubbleSort extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        float nextBarY = HEIGHT - bar_height[nextIndex]; // Adjusting the next bar's Y coordinate
        bar = new Rectangle2D.Float(nextIndex * BAR_WIDTH, nextBarY, BAR_WIDTH, bar_height[nextIndex]);
        g2d.fill(bar);
                ...
                for (int i = 0; i < SIZE - 1; i++) {
                    for (int j = 0; j < SIZE - i - 1; j++) {
                        ...
                        if (bar_height[j] > bar_height[j + 1]) {
                            swap(j, j + 1);
                            Thread.sleep(sortingDelay); // Introduce a delay to visualize the sorting process
                        }
                        repaint();
                    }
                }
